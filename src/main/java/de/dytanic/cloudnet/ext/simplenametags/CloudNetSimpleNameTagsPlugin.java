...
public final class CloudNetSimpleNameTagsPlugin extends JavaPlugin {
  ...
  private void addTeamEntry(Player target, Player all, IPermissionGroup permissionGroup, int highestSortIdLength) {
    ...
    String clantag = "";
    if (!PlaceholderAPI.setPlaceholders(target, "%clantagprovider_clantag%").equals("")) {
      clantag ="§8[§6" + PlaceholderAPI.setPlaceholders(target, "%clantagprovider_clantag%") + "§8]";
    }
    String displacname = color.replace("&", "§") + target.getDisplayName().replaceAll("&", "§").replaceAll("§7", "");



    target.setPlayerListName(prefix.replaceAll("&", "§") + displacname + "§b §r" + clantag);
    try {
